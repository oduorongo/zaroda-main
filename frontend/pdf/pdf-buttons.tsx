...
export function ReportCardButton({
  ...
  return (
    <PdfButton
      ...
      label={compact ? 'Report Card' : '↓ Download Report Card'}
      compact={compact}
      onClick={() => download(
        `/api/v1/pdf/report-card/${learnerId}?term=${term}&academicYear=${encodeURIComponent(academicYear)}`,
        `report-card-${learnerName?.replace(/\s+/g,'-') || learnerId}-${term}.pdf`,
        key,
      )}
    />
  );
...
export function BulkReportCardsButton({
  ...
  return (
    <PdfButton
      loading={downloading === key}
      label="↓ All Report Cards (PDF)"
      onClick={() => download(
        `/api/v1/pdf/report-cards/bulk?streamId=${streamId}&term=${term}&academicYear=${encodeURIComponent(academicYear)}`,
        `report-cards-${streamName?.replace(/\s+/g,'-') || streamId}-${term}.pdf`,
        key,
      )}
    />
  );
...
export function ReceiptButton({
  ...
  return (
    <PdfButton
      ...
      variant="ghost"
      onClick={() => download(
        `/api/v1/pdf/receipt/${receiptNumber}`,
...
export function BibSheetButton({
  ...
  return (
    <PdfButton
      ...
      onClick={() => download(
        `/api/v1/pdf/bib-sheet/${championshipId}${schoolId ? `?schoolId=${schoolId}` : ''}`,
...
export function PayslipButton({
  ...
  return (
    <PdfButton
      ...
      variant="ghost"
      onClick={() => download(
        `/api/v1/pdf/payslip/${staffId}?periodId=${periodId}`,
  ...
  return (
    <div className="flex items-center justify-between bg-[#f4f6fb] border border-gray-100 rounded-xl p-4">
      ...
      <PdfButton
        ...
        onClick={() => download(
          `/api/v1/pdf/report-cards/bulk?streamId=${stream.id}&term=${term}&academicYear=${encodeURIComponent(academicYear)}`,
