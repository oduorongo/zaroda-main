...
// could end up with e.g. 120/100. Clamps as the teacher types rather than only on save.
...
export default function AdminEnterMarksPage() {
  ...
  // one, instead of showing whatever the first area happens to be on page load.
  ...
  // Combined percent/level across Paper 1 + Paper 2 (used for the on-screen preview only —
  // the backend re-sums raw/max scores itself when building the mark list). Each paper can
  // have its own "out of" total.
          ...
          <button
            onClick={() => {
              ...
              const rows = paperMode
                ? [['Learner','Admission','Paper 1','Out of P1','Paper 2','Out of P2','Percent','Level']]
                : [['Learner','Admission','Score','Out of','Percent','Level']];
              filteredLearners.forEach(l => {
                const raw = areaScores[l.id] ?? '';
                if (paperMode) {
                  const raw2 = areaScoresP2[l.id] ?? '';
                  const lvl = combinedLevelFor(raw, raw2, areaMaxP1Num, areaMaxP2Num);
                  const n1 = Number(raw), n2 = Number(raw2);
                  const has1 = raw !== '' && !isNaN(n1), has2 = raw2 !== '' && !isNaN(n2);
                  const pct = (has1 || has2)
                    ? Math.round((((has1?n1:0)+(has2?n2:0)) / ((has1?areaMaxP1Num:0)+(has2?areaMaxP2Num:0))) * 100)
                    : '';
                  rows.push([`${l.firstName||''} ${l.lastName||''}`.trim(), l.admissionNumber||'', String(raw), String(areaMaxP1Num), String(raw2), String(areaMaxP2Num), pct === '' ? '' : `${pct}%`, lvl?.code || '']);
                  return;
                }
