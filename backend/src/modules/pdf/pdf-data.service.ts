...
export class PdfDataService {
  ...
  // Canonical learning areas for a grade — delegates to the shared util so every
  // report-card/mark-list renderer in the app agrees on the same subject list.
  private getGradeLearningAreas(gradeLevel: string, tenantId: string): Promise<string[]> {
    return getGradeLearningAreasShared(this.dataSource, gradeLevel, tenantId);
  }
